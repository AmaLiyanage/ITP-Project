...
const API_URL =
  import.meta.env.MODE === "development"
    ? "http://localhost:5000/api/auth"  // If in development mode, use the local backend URL
    : "/api/auth";  // If in production mode, use the relative API URL


axios.defaults.withCredentials = true;

// Create a Zustand store for handling authentication state and actions
export const useAuthStore = create((set) => ({
  ...
  signup: async (email, password, name) => {
    ...
    try {
      // Make a POST request to the signup endpoint with email, password, and name
  ...
  updateUser: async (updatedData) => {
    set({ isLoading: true, error: null });  
    try {
      // Make a PUT request to the update-profile endpoint with the updated user data
