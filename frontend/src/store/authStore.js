...
export const useAuthStore = create((set) => ({
  ...
  // Sign Up
  signup: async (email, password, name, isAdmin = false) => {
    set({ isLoading: true, error: null });
    try {
      const response = await axios.post(`${API_URL}/signup`, { email, password, name, isAdmin });
  ...
  login: async (email, password, isAdmin = false) => {
    set({ isLoading: true, error: null });
    try {
      const response = await axios.post(`${API_URL}/login`, { email, password, isAdmin });
